...
|- *analysis/*
  |-*01.fastq_processing/*
    ...
    |-*bwa_mem/*
      |-{sample}_mkdup.RG.bam
    |-*cov/*
      |-covSummary.bedtools.txt
    |-*vcf/*
      |-comm_lines.txt
      |-*raw_genotypes_HC/*
        |-{sample}.raw.snps.indels.g.vcf
      |-*called_genotypes_HC/*
        |-*{#}_database/*
          |-callset.json
          |-'CM031199.1$1$14000000'
          |-
      |-*unfilt_genotypes_HC/*
        |-{#}_HC_genotypes_unfiltered.vcf.gz
        |-{#}_HC_genotypes_unfiltered.vcf.gz.tbi
      |-*mk_dp0_as_missing/*
        |-{#}_HC_genotypes_unfiltered.mrkmiss.vcf.gz
        |-{#}_HC_genotypes_unfiltered.mrkmiss.vcf.gz.tbi
|- *resources*
  |-*reference*
    |-genome.fasta
  |-*adapters*
...
If your file structure ends up looking different (aka, you set things up differently) you will need to adjust paths within the scripts below to get them to work.
...
This is all set up to run on the test data generated by eric anderson for his snakemake workflow.
...
gunzip mini-test-genome.fna.gz
bwa index  mini-test-genome.fna
samtools faidx mini-test-genome.fna
```
...
```{bash, label="assemble-scaffold-lists.awk"}
...
mkdir -p vcf
samtools view -H bwa_mem/199967_mkdup.RG.bam | sed 's/SN://g; s/LN://g' | awk '/^@SQ/' | awk -f assemble-scaffold-lists.awk > vcf/comm_lines.txt
```
...
```{bash, label="04c.call_genotypes.sbatch"}
#!/bin/bash 
#
#SBATCH --job-name=genomedb
#SBATCH --output=genomedb.%j.out
...
And this is a last step that is required because GATK calls missing genotypes as homozygous instead of leaving them as missing, which biases your data later. So this fixes that by manually marking them as missing.

```{bash, label="04d.mark_dp0_missing.sbatch"}
#!/bin/bash
#
#SBATCH --job-name=mrkmiss
#SBATCH --output=mrkmiss.%j.out
#SBATCH --error=mrkmiss.%j.err
#################
#SBATCH -t 1:00:00
#SBATCH -p amilan
#SBATCH --qos=normal
...
mkdir -p ../mk_dp0_as_missing

bcftools +setGT ${vcf}_HC_genotypes_unfiltered.vcf.gz -- -t q -n . -i 'FMT/DP=0 | (FMT/PL[:0]=0 & FMT/PL[:1]=0 & FMT/PL[:2]=0)' | bcftools +fill-tags - -- -t 'NMISS=N_MISSING' | bcftools view -Oz - > ../mk_dp0_as_missing/${vcf}_HC_genotypes_unfiltered.mrkmiss.vcf.gz

bcftools index -t ../mk_dp0_as_missing/${vcf}_HC_genotypes_unfiltered.mrkmiss.vcf.gz
```

## Filtering
There are lots of different ways to filter, and different threshold and filters to apply depending on what you're going to do with the data. Below is a generic script, with somewhat standard filters. Review Hemstrom et al 2024 "Next-generation data filtering in the genomics era" for a nice review of filtering steps and threshold depending on the downstream analyses being run.

```{bash, label="05.merge_hardVQSR_filter.sbatch"}
#!/bin/bash
#
...
#################
#SBATCH --mail-type=END
#SBATCH  --mail-user=ericacnr@rams.colostate.edu
##################
#echo commands to stdout
set -x
...
##Location of fasta file and
REFERENCE="resources/reference/mini-test-genome.fna"
IN="vcf/mk_dp0_as_missing"
OUT="vcf/filtered_HC"

conda activate bioinf
###Merge vcfs of gatk run
picard MergeVcfs \
 $(printf ' -I %s' $IN/0*vcf.gz) \
--OUTPUT $IN/merged_gatk.vcf.gz
...
java -Djava.io.tmpdir=/scratch/alpine/ericacnr@colostate.edu/tmp -Xmx37g -jar /curc/sw/install/bio/gatk/4.3.0.0/gatk-package-4.3.0.0-local.jar SelectVariants \
     -R $REFERENCE \
     -V $IN/merged_gatk.vcf \
     -select-type SNP \
     --O $OUT/merged_gatk.SNP.vcf
...
vcftools --vcf merged_gatk.SNP.vcf --out merged_gatk.SNP.05maf --min-alleles 2 --max-alleles 2 --max-missing 0.5 --maf 0.05 --recode
...
-R $REFERENCE \
-V merged_gatk.SNP.05maf.recode.vcf \
...
-O merged_gatk.SNP.filtered_gatkVQSR2.vcf

#Exclude filtered variants (PASS= pass filters)
java -Djava.io.tmpdir=/scratch/alpine/ericacnr@colostate.edu/tmp -Xmx37g -jar /curc/sw/install/bio/gatk/4.3.0.0/gatk-package-4.3.0.0-local.jar  SelectVariants \
     -R $REFERENCE \
     -V merged_gatk.SNP.filtered_gatkVQSR2.vcf \
     --exclude-filtered  true \
     -O merged_gatk.SNP.filtered_gatkVQSR2.PASS.vcf
...
vcftools --vcf merged_gatk.SNP.filtered_gatkVQSR2.PASS.vcf --out merged_gatk.SNP.filtered_gatkVQSR2.PASS.8miss --max-missing 0.8 --recode
```
